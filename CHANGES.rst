...
- Merged scipion-chem-netmhciipan (MHC-II/T-helper, NetMHCIIpan-4.3) into
  ...
  was no technical reason to keep them as two separate plugins (review
  feedback from Blanca, CNB Biocomputing unit). The two protocols
  themselves stay independent (see each docstring for why), and using
  only one does not require the other tool to be configured.
